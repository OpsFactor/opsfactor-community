...
@Getter
public class CalendarioSimples implements Calendario {

    /** Expõe a janela uniforme pelo mesmo contrato usado pelos calendários mistos. */
    @Override
    public PeriodoCalendario getPeriodo(int posicaoPeriodo) {

        return new PeriodoCalendario(posicaoPeriodo, getPrimeiraDataHorarioPeriodo(posicaoPeriodo),
                getUltimaDataHorarioPeriodo(posicaoPeriodo), tamanhoBucket);

    }

    /** A âncora não altera a conversão histórica de dias no calendário uniforme. */
    @Override
    public double converteDiasParaPeriodosCalendario(double numeroDias, int posicaoPeriodoReferencia) {

        return converteDiasParaPeriodosCalendario(numeroDias);

    }

    /** Preserva os fatores financeiros históricos para buckets uniformes completos. */
    @Override
    public double getNumeroPeriodosNoAno(int posicaoPeriodo) {

        return getNumeroMedioPeriodosNoAno();

    }

    /**
     * Limita o futuro mantendo a implementação uniforme e sua aritmética legada.
     * Uma janela de calendário simples não deve virar um adaptador genérico:
     * Demand/split e regras de compatibilidade precisam continuar reconhecendo
     * o contrato concreto, inclusive as conversões por duração média.
     */
    @Override
    public CalendarioSimples comHorizonteFuturo(int numeroPeriodosFuturos) {

        if (numeroPeriodosFuturos <= 0 || numeroPeriodosFuturos > getNumeroPeriodosFuturos()) {
            throw new IllegalArgumentException("Horizonte da janela deve estar entre 1 e o horizonte do calendário");
        }
        return criaCalendarioDeOffsetsPeriodos(tamanhoBucket, dataHorarioInicialPresente,
                numeroPeriodosPassadosAdicional, numeroPeriodosPassados, numeroPeriodosFuturos, 0);

    }

    /** Preserva passado existente ou acrescenta uma posição anterior ao presente. */
    @Override
    public CalendarioSimples comPeriodoPassado() {

        if (getNumeroPeriodosPassadosTotal() > 0) {
            return this;
        }
        return criaCalendarioDeOffsetsPeriodos(tamanhoBucket, dataHorarioInicialPresente,
                0, 1, numeroPeriodosFuturos, numeroPeriodosFuturosAdicional);
    ...
    public static CalendarioSimples criaCalendarioDeOffsetsDias(
     ...
     * @return 
     */
    public static CalendarioSimples criaCalendarioPeriodosFuturosDeDatas(Constantes.TamanhoBucket tamanhoBucket,
    ...
    private static CalendarioSimples criaCalendarioVazio(Constantes.TamanhoBucket tamanhoBucket, LocalDateTime dataReferencia) {
        
        if (tamanhoBucket == null) throw new IllegalArgumentException("Bucket size is null");
        
        CalendarioSimples calendario = new CalendarioSimples();
    ...
    public static CalendarioSimples criaCalendarioDeDatas(Constantes.TamanhoBucket tamanhoBucket,
    ...
    public static CalendarioSimples criaCalendarioPeriodosPassadosDeDatas(Constantes.TamanhoBucket tamanhoBucket,
        ...
        LocalDateTime dataInicialFuturo = CalendarioSimples.getPrimeiraDataHorarioPeriodoCalendarioComOffset(dataFinalPassada, 1, tamanhoBucket);
     ...
     * @return 
     */
    public static CalendarioSimples criaCalendarioDeOffsetsPeriodos(
        ...
        CalendarioSimples calendario = criaCalendarioVazio(tamanhoBucket, dataHorarioReferencia);
        calendario.tamanhoBucket = tamanhoBucket;
     ...
     * @param outroCalendarioSimples
     * @param posicaoPeriodoOutroCalendario
     * @return 
     */
    public int getPosicaoPeriodoDePosicaoPeriodoOutroCalendario(CalendarioSimples outroCalendarioSimples, int posicaoPeriodoOutroCalendario) {
        
        return getPosicaoPeriodo(outroCalendarioSimples.getPrimeiraDataHorarioPeriodo(posicaoPeriodoOutroCalendario + 1).minusNanos(1));
    ...
    public double getNumeroPeriodosNoBucketReferencia(int posicaoPeriodo, TamanhoBucket tamanhoBucketReferencia) {
        ...
        if (comparacaoAgregacaoCalendarioReferencia == 1) {
            ...
            CalendarioSimples calendarioReferencia = CalendarioSimples.criaCalendarioPeriodosFuturosDeDatas(tamanhoBucketReferencia, dataHorarioInicialPeriodo, dataHorarioInicialAposPeriodo.minusNanos(1));
    ...
    public double consolidaDadosNoCalendario(int posicaoPeriodoCalendario, TamanhoBucket tamanhoBucketDados, ToDoubleFunction<LocalDateTime> valorPorDataHorarioDados) {
        ...
        // nesse caso, chamar a função uma vez para cada ocorrência dentro do período calendário
            ...
            CalendarioSimples calendarioDados = criaCalendarioPeriodosFuturosDeDatas(tamanhoBucketDados, primeiraDataHorarioPosicaoPeriodo, primeiraDataHorarioAposPosicaoPeriodo.minusNanos(1));
            // um localdatetime no bucket dos dados é necessário para se aplicar a função
            LocalDateTime primeiraDataHorarioPeriodoDados = getPrimeiraDataHorarioPeriodoCalendarioComOffset(primeiraDataHorarioPosicaoPeriodo, 0, tamanhoBucketDados);
    ...
    public Map<Integer, Double> getNumeroHorasPorPeriodoDeRangeDatasHorarios(
        ...
        double numeroHorasNoPrimeiroPeriodo = Duration
                .between(
                        dataHorarioInicial,
                        CalendarioSimples.getMinDataHorario(getUltimaDataHorarioPeriodo(posicaoPeriodoInicio), dataHorarioFinal))
                .toSeconds() / 3600d;
        double numeroHorasNoUltimoPeriodo = Duration
                .between(
                        CalendarioSimples.getMaxDataHorario(getPrimeiraDataHorarioPeriodo(posicaoPeriodoFim), dataHorarioInicial),
                        dataHorarioFinal)
                .toSeconds() / 3600d;
